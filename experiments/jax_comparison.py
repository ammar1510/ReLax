...
def convert_weights(torch_weights, jax_config: ModelConfig):
    ...
    # Embedding
    jax_params["params"]["tok_embeddings"] = {
        "embedding": jnp.asarray(
            torch_weights["tok_embeddings.weight"], dtype=jax_config.dtype
        )
    }
    ...
    for i in range(jax_config.n_layers):
        ...
        # Attention weights
        wq = jnp.asarray(
            torch_weights[layer_prefix + "attention.wq.weight"], dtype=jax_config.dtype
        ).T.reshape(jax_config.dim, jax_config.n_heads, jax_config.head_dim)
        wk = jnp.asarray(
            torch_weights[layer_prefix + "attention.wk.weight"], dtype=jax_config.dtype
        ).T.reshape(jax_config.dim, jax_config.n_kv_heads, jax_config.head_dim)
        wv = jnp.asarray(
            torch_weights[layer_prefix + "attention.wv.weight"], dtype=jax_config.dtype
        ).T.reshape(jax_config.dim, jax_config.n_kv_heads, jax_config.head_dim)
        wo = jnp.asarray(
            torch_weights[layer_prefix + "attention.wo.weight"], dtype=jax_config.dtype
        ).T
        ...
        w1 = jnp.asarray(
            torch_weights[layer_prefix + "feed_forward.w1.weight"],
            dtype=jax_config.dtype,
        ).T
        w2 = jnp.asarray(
            torch_weights[layer_prefix + "feed_forward.w2.weight"],
            dtype=jax_config.dtype,
        ).T
        w3 = jnp.asarray(
            torch_weights[layer_prefix + "feed_forward.w3.weight"],
            dtype=jax_config.dtype,
        ).T
        ...
        jax_layer["attention_norm_weight"] = jnp.asarray(
            torch_weights[layer_prefix + "attention_norm.weight"],
            dtype=jax_config.dtype,
        )
        jax_layer["ffn_norm_weight"] = jnp.asarray(
            torch_weights[layer_prefix + "ffn_norm.weight"], dtype=jax_config.dtype
        )

        jax_params["params"][f"layer_{i}"] = flax.core.freeze(jax_layer)

    # Final normalization
    jax_params["params"]["norm_weight"] = jnp.asarray(
        torch_weights["norm.weight"], dtype=jax_config.dtype
    )
