...
def test_jit_rms_norm():
    ...
    np.testing.assert_allclose(np.array(output_eager), np.array(output_jitted), rtol=1e-5)
...
def test_jit_apply_rotary_emb():
    ...
    # First call
    x = jax.random.normal(jax.random.PRNGKey(0), (BSZ, SEQLEN, N_HEADS, HEAD_DIM), dtype=DTYPE)
    output_eager = apply_rotary_emb(x, freqs_cis[:SEQLEN])
    output_jitted = jitted_apply_rotary(x, freqs_cis[:SEQLEN])
    np.testing.assert_allclose(np.array(output_eager), np.array(output_jitted), rtol=5e-4)

    # Subsequent calls
    for i in range(1, 3):
        x = jax.random.normal(jax.random.PRNGKey(i), (BSZ, SEQLEN, N_HEADS, HEAD_DIM), dtype=DTYPE)
...
def test_jit_repeat_kv():
    ...
    # First call
    x = jax.random.normal(jax.random.PRNGKey(0), (BSZ, SEQLEN, N_KV_HEADS, HEAD_DIM), dtype=DTYPE)
    ...
    # Subsequent calls
    for i in range(1, 3):
        x = jax.random.normal(jax.random.PRNGKey(i), (BSZ, SEQLEN, N_KV_HEADS, HEAD_DIM), dtype=DTYPE)
...
def test_jit_feed_forward():
    ...
    np.testing.assert_allclose(np.array(output_eager), np.array(output_jitted), rtol=1e-5, atol=1e-5)
...
def test_jit_attention_prefill():
    ...
    jitted_gqa = jax.jit(
        chex.assert_max_traces(grouped_query_attention, n=1)
    )
    ...
    np.testing.assert_allclose(np.array(output_eager), np.array(output_jitted), rtol=5e-3, atol=1e-3)
    np.testing.assert_allclose(np.array(cache_eager.k), np.array(cache_jitted.k), rtol=5e-3, atol=1e-3)
    np.testing.assert_allclose(np.array(cache_eager.v), np.array(cache_jitted.v), rtol=5e-3, atol=1e-3)
...
def test_jit_attention_decode():
    ...
    kv_cache = KVCache.new(1, BSZ, MAX_SEQLEN, N_KV_HEADS, HEAD_DIM, dtype=DTYPE)
    prefill_k = jax.random.normal(key_cache, (1, BSZ, start_pos, N_KV_HEADS, HEAD_DIM), dtype=DTYPE)
    prefill_v = jax.random.normal(key_cache, (1, BSZ, start_pos, N_KV_HEADS, HEAD_DIM), dtype=DTYPE)
    ...
    np.testing.assert_allclose(np.array(output_eager), np.array(output_jitted), rtol=1e-5, atol=1e-5)
    np.testing.assert_allclose(np.array(cache_eager.k), np.array(cache_jitted.k), rtol=1e-5, atol=1e-5)
    np.testing.assert_allclose(np.array(cache_eager.v), np.array(cache_jitted.v), rtol=1e-5, atol=1e-5)
    ...
    for i in range(1, 3):
        ...
        _, _ = jitted_gqa(
            x_new, freqs_cis, params, prefilled_cache, layer_idx, start_pos, prefill_mask=None
        ) 