...
class TestKVCache:
    ...
    def test_update_cache(self):
        ...
        xk_update_val = (
            jnp.arange(bsz * seqlen_update * kv_heads * head_dim, dtype=dtype).reshape(
                bsz, seqlen_update, kv_heads, head_dim
            )
            + 1.0
        )  # Add 1 to distinguish from initial zeros
        xv_update_val = (
            jnp.arange(bsz * seqlen_update * kv_heads * head_dim, dtype=dtype).reshape(
                bsz, seqlen_update, kv_heads, head_dim
            )
            + 10.0
        )  # Add 10 to distinguish from xk and initial zeros
        ...
        if start_pos + seqlen_update < max_seq_len:
            assert jnp.all(
                updated_cache.k[layer_idx, :, start_pos + seqlen_update :, :, :] == 0
            )
            assert jnp.all(
                updated_cache.v[layer_idx, :, start_pos + seqlen_update :, :, :] == 0
            )
    ...
    def test_get_layer(self):
        ...
        cache = cache.update(
            xk_update_val, xv_update_val, layer_idx_to_update, start_pos_update
        )
        ...
        np.testing.assert_array_equal(
            retrieved_k[:, :seqlen_update, :, :], xk_update_val
        )
        ...
        np.testing.assert_array_equal(
            retrieved_v[:, :seqlen_update, :, :], xv_update_val
        )
