...
def test_transformer_forward_pass():
    ...
    torch_args = ModelArgs(
        ...
        vocab_size=vocab_size, multiple_of=multiple_of, max_seqlen=max_seqlen,
    ...
    jax_args = ModelConfig(
        dim=dim, n_layers=n_layers, n_heads=n_heads, n_kv_heads=n_kv_heads, 
        vocab_size=vocab_size, ffn_hidden_dim=ffn_hidden_dim, max_seqlen=max_seqlen,
    ...
    tokens_jax, tokens_torch = jnp.array(tokens_np), torch.tensor(tokens_np,device=torch_device)
    ...
    # Final Comparison
    np.testing.assert_allclose(logits_jax, logits_torch.cpu().detach().numpy(), rtol=1e-2, atol=1e-2)
